...
SC.audio = (function () {
  ...
  const IA = 'https://ia600601.us.archive.org/31/items/lostinthe-night-1/';
  const RADIO_TRACKS = [
    { id: 'ia_full_speed',    title: 'Full Speed Ahead',     url: IA + 'Full%2BSpeed%2BAhead.mp3' },
    { id: 'ia_lost_night_2',  title: 'Lost in the Night II',  url: IA + 'Lost%2Bin%2Bthe%2BNight%20%281%29.mp3' },
    { id: 'ia_lost_night',    title: 'Lost in the Night',     url: IA + 'Lost%2Bin%2Bthe%2BNight.mp3' },
    { id: 'ia_lost_velocity', title: 'Lost in the Velocity',  url: IA + 'Lost%2Bin%2Bthe%2BVelocity.mp3' },
    { id: 'ia_zero_latency',  title: 'Zero Latency',          url: IA + 'Zero%2BLatency.mp3' }
  ];

  /* موسيقى شاشة البداية: مقطع واحد يدور خلف سباق الاستعراض، ويُحفظ
     للعمل دون إنترنت مثل بقيّة المقاطع. */
  const MENU_TRACK = { id: 'ia_born_for_the_rush', title: 'Born for the Rush', kind: 'url',
    url: IA + 'Born%2Bfor%2Bthe%2BRush.mp3' };
  ...
  /* امسح ما لم يعد ضمن القائمة: المقاطع المحذوفة بقيت تعمل دون إنترنت
     من الذاكرة المحفوظة، فلا بدّ من إزالتها من جهاز اللاعب أيضاً. */
  function pruneCache() {
    const keep = new Set(buildStations().filter((s) => s.kind === 'url').map((s) => s.id));
    keep.add(MENU_TRACK.id);
    return openDB().then((db) => new Promise((res) => {
      if (!db) return res(0);
      try {
        ...
        rq.onsuccess = () => {
          let n = 0;
          (rq.result || []).forEach((k) => {
            if (!keep.has(k)) { store.delete(k); delete radio.cached[k]; n++; }
          });
          res(n);
        };
        rq.onerror = () => res(0);
  ...
  function radioSet(id) {
    ...
    if (i >= 0) return playStation(i);
    /* محطّة محفوظة لم تعد موجودة (تغيّرت المقاطع): ابدأ بأوّل مقطع */
    const first = radio.stations.findIndex((s) => s.kind === 'url');
    return playStation(first < 0 ? 1 : first);
  }
  ...
  /* حالة مفصّلة للتشخيص: هل العنصر يعمل فعلاً ولماذا لا يعمل */
  function radioState() {
    const st = currentStation();
    return { id: st.id, kind: st.kind, on: radio.on, loading: radio.loading,
             paused: radio.el ? radio.el.paused : null,
             vol: radio.el ? +radio.el.volume.toFixed(2) : null,
             target: +radioVolume().toFixed(2), hasSrc: !!(radio.el && radio.el.src),
             tried: radio.tried || 0, lastErr: radio.lastErr || null };
  }
  ...
  function radioResume() {
    ...
    if (pr && pr.catch) pr.catch((e) => {
      radio.lastErr = (e && e.name) || 'play-failed';
      radioWaitTap();
    });
  }
  function radioWaitTap() {
    if (radio.waiting) return;
    radio.waiting = true;
    const once = () => {
