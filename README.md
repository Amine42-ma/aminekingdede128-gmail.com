...
`index.html` مقسّم إلى خمسة عشر جزءًا متسلسلة قابلة للدمج مباشرة:

| الجزء | المحتوى |
|---|---|
| **PART 1/15** | الترويسة، import map، نظام التصميم (mobile-first، safe-area، 100dvh) |
| **PART 2/15** | هيكل الصفحة وطقم الواجهة (i18n، Sheets، Modals، Toasts) |
| **PART 3/15** | طبقة البيانات: Firebase (Auth/Firestore/Storage) + بديل محلي معلن + قواعد الأمان |
| **PART 4/15** | نواة المحرك: العارض، شجرة الكائنات، المحمّلات والذاكرة، لمس التحرير، الفيزياء |
| **PART 5/15** | Engine API + تشغيل السكربتات في نطاق مقيّد + Mic + Network + Voice + التوثيق |
| **PART 6/15** | خط الأصول: تحليل، صور مصغّرة، معاينة 3D، المكتبة والبحث |
| **PART 7/15** | الاستوديو: الشجرة، الخصائص، العالم، مساعد AI، التشغيل والنشر |
| **PART 9/15** | نظام البناء: Toolchain Manager، Language Adapters، Build Manager، Compiler Cache، Engine Bridge (WASM) |
| **PART 10/15** | بيئة التطوير متعدّدة اللغات: الملفات، التبويبات، BUILD/RUN/STOP، Problems، Toolchains |
| **PART 11/15** | المشاريع: عرض/حذف/تكرار، نقاط الرجوع والتاريخ، سياق المساعد وصلاحياته، لوحة الأوامر، نظام الشرح |
| **PART 12/15** | مكتبة الأنظمة وبناة العوالم، المخطّط، المنفّذ، حلقة البناء/الاختبار/الإصلاح، الفاحص |
| **PART 13/15** | وضع المبتدئ: البطاقات، المعالج، القوالب + لوحة المساعد |
| **PART 14/15** | المجتمع — البيانات: المنشورات والقصص والفيديو والمتابعة والتنبيهات والبلاغات والبث + قواعد الأمان |
| **PART 15/15** | المجتمع — الواجهة: الخلاصة، المحرّر، التسجيل، المشغّل، الملفات، صفحات الألعاب، البث، محتواي |
| **PART 8/15** | المنصة: الرئيسية، إنشاء، مشاريعي، المجتمع، الحساب + المشغّل + الإقلاع |
