...
| المشروع | الملف | ماذا يفعل |
|---|---|---|
| **حكاية · HIKAYA** | [`hikaya.html`](hikaya.html) | يؤلّف قصة ويخرجها **فيلماً** — صورة سينمائية وحواراً منطوقاً وموسيقى ومؤثرات، ثم يصدّره فيديو |
| **صوتك · Sawtak** | [`index.html`](index.html) | يدبلج **فيلماً موجوداً** إلى لغة أخرى بأصوات بشرية |
...
# حكاية · HIKAYA — استوديو توليد الأفلام

ملف واحد: [`hikaya.html`](hikaya.html). افتحه في المتصفح مباشرة — لا تثبيت ولا خادم ولا إنترنت.

## ماذا يفعل

يكتب قصة كاملة بثلاثة فصول (أو يأخذ قصتك أنت)، يقسّمها إلى مشاهد ولقطات، ثم **يخرجها على الشاشة**:

- **صورة سينمائية مُولَّدة**: ١٨ موقعاً (مدينة، صحراء، غابة، بحر، فضاء، محطة، ممر، غرفة، أطلال، معبد، جبل، كهف، جسر، طريق، ثلج، حقل، سطح، زقاق)، و١٦ حالة إضاءة، وطقس (مطر، ثلج، غبار، ضباب، جمرات، رماد، نجوم)، وطبقات عمق تتحرك مع الكاميرا.
- **إخراج**: حجم اللقطة (واسعة، متوسطة، قريبة، قريبة جداً) وحركة الكاميرا (تقدّم، تراجع، مسح، رافعة، محمولة باليد) لكل لقطة.
- **معالجة لونية**: ١٠ أساليب صورة، حبيبات فيلم، توهّج، هالة ضوء، انزياح لوني، تعتيم أطراف، أشرطة سينمائية (٢.٣٩ / ١.٨٥ / ١٦:٩ / ٤:٣).
- **أداء صوتي**: صوت لكل شخصية، مع اختيار اللغة واللهجة والطبقة والسرعة، وراوٍ يصف المشاهد.
- **موسيقى تصويرية تُؤلَّف لحظياً**: ٩ طوابع (سينث، رهبة، نبض، ملحمي، بيانو، وتريات، كورال، جاز، خفيف) تتغيّر شدّتها مع تصاعد الحبكة، مع خفض تلقائي أثناء الحوار.
- **أجواء ومؤثرات**: مطر، ريح، مدينة، بحر، محرّك، دقات قلب، رنين، ارتطام، صعود، رعد…
- **تصدير**: الفيلم فيديو WebM، وملف ترجمة SRT، والسيناريو نصاً، وملصق الفيلم PNG، والمشروع JSON لاستعادته لاحقاً.

## الاستخدام

1. اكتب فكرة — أو اتركها فارغة — واختر النوع والمزاج واللغة والمدة.
2. اضغط **«اكتب السيناريو وأخرِج الفيلم»**.
3. عدّل ما تشاء: أي لقطة (مكانها، إضاءتها، كاميراتها، حوارها) من تبويب «اللقطة»، وأي صوت من تبويب «الأصوات».
4. صدّر من زر **«تصدير»**.

في وضع **«قصتي أنا»** اكتب نصك مباشرة: سطر لكل جملة، والحوار هكذا `الاسم: الكلام`. يستخرج الاستوديو الشخصيات، ويستنتج المواقع والإضاءة من كلماتك (ليل، مطر، صحراء، بحر، سطح…)، ويبني الشريط الصوتي.

## حدود صادقة

الصورة هنا **سينما مُصمَّمة**: ظلال وسيلويت وطبقات عمق وإضاءة — وليست تصويراً واقعياً بوجوه بشرية. توليد فيديو واقعي يحتاج نماذج ضخمة تعمل على خوادم، ولا يمكن أن يسكن داخل ملف HTML واحد. ما عدا ذلك، كل ما تراه وتسمعه يُصنع داخل متصفحك.

- **الأصوات** تأتي من نظام تشغيلك عبر المتصفح؛ كلما ثبّتَ حزم لغات أكثر زادت اللهجات المتاحة.
- **«تسجيل مع الأصوات»** يطلب مشاركة صوت التبويب، لأن أصوات النطق لا تمرّ عبر مازج الصوت. و«تسجيل بالموسيقى فقط» يعمل بلا إذن لكن بلا حوار.
- **المحرّك الداخلي** يكتب بالعربية أو الإنجليزية بلا إنترنت. ولكتابة أعمق أو بلغة أخرى، اربط نموذجاً لغوياً بمفتاحك من زر «المحرّك» — المفتاح يبقى في `sessionStorage` داخل متصفحك.

## اختصارات

`مسافة` تشغيل/إيقاف · `→ ←` تقديم وتأخير · `F` ملء الشاشة
